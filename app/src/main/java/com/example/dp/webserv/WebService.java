...
public class WebService extends Service implements WebServer.OnWebServListener {
    ...
    public void onCreate() {
        super.onCreate();
        if (DEBUG) {
            Log.d(TAG, String.format("create server: port=%d, root=%s", Config.PORT, Config.WEBROOT));
        }
    ...
    private void restartResetTask(long delay) {
        ...
        resetTask = new TimerTask() {
            ...
            public void run() {
                ...
                resetTask = null;
                if (DEBUG)
                    Log.d(TAG, "ResetTask executed.ccccsdsk");
            }
        };
    ...
    public void setOnWebServListener(WebServer.OnWebServListener mListener) {
        ...
        Log.d(TAG, "setOnWebServListener = " + mListener);
    }
